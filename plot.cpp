...
void Plot::QPlot_init(QCustomPlot *customPlot)
{
    ...
    customPlot->yAxis->ticker()->setTickCount(11); // 11个主刻度
...
void Plot::showDashboard(QCustomPlot *customPlot)
{
    ...
    for (size_t j = 0; j < sizeof(defaultLegendIndex) / sizeof(defaultLegendIndex[0]); ++j)
    {
...
void Plot::mouseMove2(QMouseEvent *e)
{
    ...
    for (int i = 0; i < pPlot1->graphCount(); ++i)
    {
        ...
        if(pChkVisibleCurve[i]->isChecked())
    ...
    if(x4.contains(vLineX)){
        QPen DashPen(Qt::red, 1, Qt::DashLine);
        if (vLine->pen().color() != DashPen.color()) {
            vLine->setPen(DashPen);
        }
    }
    else if(x5.contains(vLineX)){
        QPen DashPen(Qt::green, 1, Qt::DashLine);
        if (vLine->pen().color() != DashPen.color()) {
            vLine->setPen(DashPen);
        }
    }
    else if(x6.contains(vLineX)){
        QPen DashPen(Qt::blue, 1, Qt::DashLine);
        if (vLine->pen().color() != DashPen.color()) {
            vLine->setPen(DashPen);
        }
    }
    else if(x7.contains(vLineX)){
    ...
    if (1)
    {
        if (1)
        {
            ...
            if (!x1.isEmpty())
            {
                ...
                rect1->topLeft->setCoords(x1.first(), pPlot1->yAxis->range().upper - FIRST_PHASE_COUNT_RANGE);
            ...
            if (!x2.isEmpty())
            {
                ...
                rect2->topLeft->setCoords(x2.first(), pPlot1->yAxis->range().upper - FIRST_PHASE_COUNT_RANGE); // 只到y=0
            ...
            if (!x3.isEmpty())
            {
                ...
                rect3->topLeft->setCoords(x3.first(), pPlot1->yAxis->range().upper - FIRST_PHASE_COUNT_RANGE); // 只到y=0
            ...
            if (!x4.isEmpty())
            {
                ...
                rect4->bottomRight->setCoords(x4.last(), pPlot1->yAxis->range().upper - FIRST_PHASE_COUNT_RANGE);
                rect4->setBrush(QBrush(QColor(255, 0, 0, FIRST_PHASE_COUNT_COLOR_DEEPTH))); // 红色
            ...
            if (!x5.isEmpty())
            {
                ...
                rect5->bottomRight->setCoords(x5.last(), pPlot1->yAxis->range().upper - FIRST_PHASE_COUNT_RANGE);
                rect5->setBrush(QBrush(QColor(0, 255, 0, FIRST_PHASE_COUNT_COLOR_DEEPTH))); // 绿色
            ...
            if (!x6.isEmpty())
            {
                ...
                rect6->bottomRight->setCoords(x6.last(), pPlot1->yAxis->range().upper - FIRST_PHASE_COUNT_RANGE);
                rect6->setBrush(QBrush(QColor(0, 0, 255, FIRST_PHASE_COUNT_COLOR_DEEPTH))); // 蓝色
            ...
            if (!x7.isEmpty())
            {
                ...
                rect7->bottomRight->setCoords(x7.last(), pPlot1->yAxis->range().upper - FIRST_PHASE_COUNT_RANGE);
                rect7->setBrush(QBrush(QColor(255, 255, 0, FIRST_PHASE_COUNT_COLOR_DEEPTH))); // 黄色
